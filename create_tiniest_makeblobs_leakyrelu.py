"""
Tiniest 8-8-8-8-8 make_blobs model — Leaky ReLU variant.
Architecture: 8 -> 8 -> 8 -> 8 -> 8 -> 8 (input, 4 hidden, output).
...
print("=" * 60)
print(f"TINIEST LeakyReLU(alpha={LEAKY_ALPHA}): {N_FEATURES} -> {' -> '.join(map(str,HIDDEN))} -> {OUT}")
print("=" * 60)

X, y = make_blobs(
    n_samples=N_SAMPLES, n_features=N_FEATURES, centers=N_CLASSES,
    cluster_std=1.6, random_state=SEED, shuffle=True,
)
...
class TiniestLeakyReLU(nn.Module):
    def __init__(self):
        super().__init__()
        ...
        self.fc5 = nn.Linear(HIDDEN[3], OUT)
        self.double()
...
net = TiniestLeakyReLU().to(device)
...
pth_path = os.path.join(OUT_DIR, "tiniest_makeblobs_leakyrelu.pth")
torch.save(net.state_dict(), pth_path)
print("Saved", pth_path)

# Keras model: explicit LeakyReLU layers (separate from Dense) so Dense weights
# remain cleanly addressable for whitebox / sign_recovery readers.
keras_model = keras.Sequential([
    keras.layers.Dense(HIDDEN[0], input_shape=(N_FEATURES,), dtype='float64', name='dense_1'),
    keras.layers.LeakyReLU(alpha=LEAKY_ALPHA, dtype='float64', name='leaky_1'),
    keras.layers.Dense(HIDDEN[1], dtype='float64', name='dense_2'),
    keras.layers.LeakyReLU(alpha=LEAKY_ALPHA, dtype='float64', name='leaky_2'),
    keras.layers.Dense(HIDDEN[2], dtype='float64', name='dense_3'),
    keras.layers.LeakyReLU(alpha=LEAKY_ALPHA, dtype='float64', name='leaky_3'),
    keras.layers.Dense(HIDDEN[3], dtype='float64', name='dense_4'),
    keras.layers.LeakyReLU(alpha=LEAKY_ALPHA, dtype='float64', name='leaky_4'),
    keras.layers.Dense(OUT, activation=None, dtype='float64', name='dense_5'),
])
net.cpu()
pw = {
...
keras_path = os.path.join(OUT_DIR, "tiniest_makeblobs_leakyrelu.keras")
keras_model.save(keras_path)
print("Saved", keras_path)

# Record alpha for downstream readers
alpha_path = os.path.join(OUT_DIR, "tiniest_makeblobs_leakyrelu_alpha.txt")
with open(alpha_path, 'w') as f:
    f.write(f"{LEAKY_ALPHA}\n")
print("Saved", alpha_path)

# Test data is identical to ReLU run (same seed/split) — only re-save for completeness
np.save(os.path.join(DATA_DIR, "x_test_tiniest_makeblobs.npy"), X_test)
np.save(os.path.join(DATA_DIR, "y_test_tiniest_makeblobs.npy"), y_test)
