"""
Evaluate the reconstructed TINY (64x5->10 makeblobs) model on the original sklearn make_blobs task.

Loads:
  - Oracle: tiny_stuff/makeblobs_relu.pth
  - Reconstructed: results/reconstructed_models/reconstructed_makeblobs.pth
  - Test split: data/x_test_makeblobs.npy, data/y_test_makeblobs.npy
  - Regenerates make_blobs with the same seed for a full-dataset eval (train + test)

Reports same metrics as the tiniest evaluator.
"""
import os
import json
import numpy as np
...
ORACLE_PATH = os.path.join(BASE, "tiny_stuff/makeblobs_relu.pth")
RECON_PATH  = os.path.join(BASE, "results/reconstructed_models/reconstructed_makeblobs.pth")
X_TEST_PATH = os.path.join(BASE, "data/x_test_makeblobs.npy")
Y_TEST_PATH = os.path.join(BASE, "data/y_test_makeblobs.npy")
...
def main():
    print("=" * 70)
    print("Reconstructed TINY (64x5->10) model on sklearn make_blobs")
    print("=" * 70)
    ...
    for name, (X, y) in sets.items():
        ...
        print(f"  reconstructed agreement        = {agreement:.4f}")
    ...
    out_path = os.path.join(BASE, "results/reconstructed_models/makeblobs_tiny_eval.json")
