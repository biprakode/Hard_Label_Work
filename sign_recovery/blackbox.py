...
def findCorner(weights, biases, shape, targetNeurons, targetValue=1, tol=1e-6,
    ...
    if type(dataset) is np.ndarray:
        x0 = dataset
    elif (dataset is None) or (dataset=='random1'):
        x0 = np.random.uniform(low=-1.0, high=1.0, size=shape).flatten()
    elif dataset=='random_high':
        x0 = np.random.uniform(low=-5.0, high=5.0, size=shape).flatten()
