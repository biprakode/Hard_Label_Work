...
def is_on_decision_boundary(point, delta):
    ...
    r = torch.randn(IDIM).cheat_net_cuda.to(device).double() * delta
    left = bmodel(point+r)
...
def is_on_decision_boundary_cheat(point, delta):
    ...
    #print(gapt(torch.tensor(point).cheat_net_cuda.double().to(device).double()))
...
def refine_to_decision_boundary_cheat(xo, tolerance=1e-13, max_iterations=10):
    x = torch.tensor(xo, dtype=torch.double, device=device)
    y = gapt(x).item()
...
def find_dual_points():
    ...
    while True:
        ...
        boundaryt = torch.tensor(boundary, dtype=torch.double, device=device)
        step_dirt = torch.tensor(step_dir, dtype=torch.double, device=device)
        for step_size in 10**np.arange(-5, 5, .1):
            ...
            #new_forward = torch.tensor(refine_to_decision_boundary(forward.cpu().numpy())).cheat_net_cuda.double().to(device)
            #step_dirt = new_forward - torch.tensor(original_boundary).cheat_net_cuda.double().to(device)
        ...
        original_boundaryt = torch.tensor(original_boundary, dtype=torch.double, device=device)
...
def main():
    ...
    if not os.path.exists(os.path.join(BASE_DIR, "signature_recovery/exp/%d")%SEED):
        os.makedirs(os.path.join(BASE_DIR, "signature_recovery/exp/%d")%SEED)
    import pickle
    pickle.dump(all_points, open(os.path.join(BASE_DIR, "signature_recovery/exp/%d/duals_%08d.p")%(SEED,random.randint(0, 1000000)),"wb"))
