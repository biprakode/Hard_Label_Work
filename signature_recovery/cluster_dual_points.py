...
def cheat_cluster(layer):
    duals = []
    root = '/run/media/biprarshi/COMMON/files/AI/hard-label-dnn-extraction/signature_recovery/exp/1'
    ...
    pickle.dump(cheating, open("/run/media/biprarshi/COMMON/files/AI/hard-label-dnn-extraction/signature_recovery/exp/1-cluster-%d.p"%layer, "wb"))
...
def cluster_slow(layer):
    ...
    for cluster_id,a in enumerate(duals[:1000]):
        ...
        pickle.dump(output, open("/run/media/biprarshi/COMMON/files/AI/hard-label-dnn-extraction/signature_recovery/exp/1-cluster/%d.p"%layer, "wb"))
